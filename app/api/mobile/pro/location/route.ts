...
export async function POST(request: NextRequest) {
  try {
    ...
    if (employeeError) {
      console.error(
        "[Mobile Pro Location] Employee lookup failed:",
    ...
    if (orderId) {
      ...
      if (orderError) {
        console.error(
          "[Mobile Pro Location] Order lookup failed:",
          orderError.message,
        );
        return mobileJson(
    ...
    if (employeeUpdateError) {
      console.error(
        "[Mobile Pro Location] Employee update failed:",
        employeeUpdateError.message,
      );
      return mobileJson(
        { success: false, error: "Failed to update pro location" },
        { status: 500 },
      );
    }
    ...
    if (orderId) {
      ...
      if (orderLocationError) {
        console.warn(
          "[Mobile Pro Location] Order location mirror failed:",
          orderLocationError.message,
        );
      }
    }
    ...
    if (eventError) {
      console.warn(
        "[Mobile Pro Location] Location event insert failed:",
        eventError.message,
      );
    }
