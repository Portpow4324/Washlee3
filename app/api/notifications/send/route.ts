...
async function sendLegacyFcm(params: {
  ...
  const serverKey = process.env.FCM_LEGACY_SERVER_KEY || process.env.FIREBASE_SERVER_KEY
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (userIds.length === 0 && directTokens.length === 0) {
      return NextResponse.json(
        { success: false, error: 'Provide at least one userId or device token' },
        { status: 400 }
      )
    }
    ...
    if (uniqueUserIds.length > 0) {
      ...
      if (notificationError) {
        console.warn('[Notifications Send] In-app insert failed:', notificationError.message)
      }
      ...
      if (tokenError) {
        console.warn('[Notifications Send] Device token lookup failed:', tokenError.message)
      } else {
    ...
    for (const token of uniqueTokens) {
      try {
        const result = await sendLegacyFcm({ token, title, body: message, data })
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        pushFailed: failed,
        pushSkipped: skipped,
        pushTransport: process.env.FCM_LEGACY_SERVER_KEY || process.env.FIREBASE_SERVER_KEY
          ? 'fcm_legacy'
          : 'not_configured',
      },
    })
